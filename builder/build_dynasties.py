...
.hero-portrait{
  width:clamp(160px,42vw,260px); height:auto; z-index:1;
  border:5px solid #141414; border-radius:22px; background:#fff;
...
.comic-scene{margin-top:12px;}
.comic-scene img{width:100%; border:4px solid #141414; border-radius:14px; box-shadow:6px 6px 0 #141414; display:block;}
...
def render_dynasty(d, idx, total):
    ...
    for p in d["process"]:
        ...
        scene = p.get("scene")
        scene_html = (f'<div class="comic-scene"><img src="{scene}" alt="{esc(p["title"])}" loading="lazy"></div>'
                      if scene else "")
        comic += f'''
